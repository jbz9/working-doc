...
1. **共享锁（Shared Lock）：**
   - 共享锁用于防止其他事务获取排他锁，但不阻止其他事务获取共享锁。多个事务可以同时持有共享锁，这表示它们可以读取共享资源，但不能修改它。共享锁通过 `SELECT ... FOR SHARE` 语句获得。
2. **排他锁（Exclusive Lock）：**
   - 排他锁用于防止其他事务获取任何类型的锁。当一个事务持有排他锁时，其他事务不能同时持有任何锁。排他锁通过 `SELECT ... FOR UPDATE` 或 `UPDATE` 语句获得。
3. **行锁（Row Lock）：**
   - 行锁用于锁定表中的特定行，以确保对该行的访问是原子的。行锁可以通过 `FOR UPDATE` 或 `FOR SHARE` 语句实现。
4. **表锁（Table Lock）：**
   - 表锁用于锁定整个表，防止其他事务访问该表。表锁的粒度较大，可能影响性能，因此在 MySQL 中通常会优先使用行锁。表锁可以通过 `LOCK TABLES` 语句获得。
5. **意向锁（Intention Lock）：**
   - 意向锁是一种辅助锁，用于指示事务将要获取何种类型的锁。有两种类型的意向锁：意向共享锁（`Intention Shared Lock`）和意向排他锁（`Intention Exclusive Lock`）。它们通过 `SHOW ENGINE INNODB STATUS` 命令查看。
6. **自旋锁（Spin Lock）：**
   - 自旋锁是一种轮询机制，事务在尝试获取锁时，如果锁已被其他事务持有，它会反复尝试获取，而不是阻塞等待。自旋锁适用于锁被短时间持有的场景。

### Mybait如何实现乐观锁

答：

**表和实体类：** 表和实体类中增加一个版本号字段

**更新数据：**在更新时，带上这个 **version**判断
...
### 如果该表存在多个字段查询频繁，是该建立多个单列索引还是创建一个多列联合索引呢？
...
```
1. 应用程序创建 SqlSessionFactory
    |
2. SqlSessionFactory 创建 SqlSession
    |
3. SqlSession 通过 MappedStatement 找到 SQL 语句
    |
4. SqlSession 通过 Executor 执行 SQL 语句
    |
5. Executor 通过 StatementHandler 处理 SQL 语句的创建和执行
    |
6. Executor 通过 ParameterHandler 处理参数的映射
    |
7. Executor 通过 ResultHandler 处理查询结果的映射
    |
8. 执行完 SQL 语句后，返回结果给应用程序
```
